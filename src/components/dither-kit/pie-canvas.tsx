...
export function PieCanvas() {
  ...
  useEffect(() => {
    ...
    const draw = (now: number) => {
      ...
      const prog =
        s.animate && !reduce
          ? Math.min(1, (now - animStart) / Math.max(s.animationDuration, 1))
          : 1
      ...
      if (s.hoverIndex !== lastHover) {
        ...
        // A newly hovered slice starts at rest; on pointer leave, retain the
        // current amount so the previous bulge can ease back down.
      ...
      const paintSig = `${s.innerRadius}|${s.pie
        .map((sl) => {
          const seed = s.seedOf(sl.name)
          return `${sl.name}:${sl.value}:${s.variantOf(sl.name)}:${seed.fill.join(",")}`
        })
        .join(",")}`
      ...
      if (bloomCtx) {
        ...
        if (on) bloomCtx.drawImage(canvas, 0, 0)
      }

      const intensityMoving = Math.abs(intensity - itTarget) > 0.001
      const popMoving = Math.abs(popEase - popTarget) > 0.001
      if (prog < 1 || intensityMoving || popMoving) {
