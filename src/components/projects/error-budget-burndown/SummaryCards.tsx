...
import { type BudgetCalculation, formatDuration } from './calculations';
...
export function SummaryCards({ calculation }: SummaryCardsProps) {
  const {
    totalBudgetMinutes,
    consumedMinutes,
    remainingMinutes,
  ...
  const burnRateText = burnMultiplier < 0.5 ? 'Very Low' :
                       burnMultiplier < 0.9 ? 'Low' :
                       burnMultiplier < 1.1 ? 'On Track' :
                       burnMultiplier < 2 ? 'High' : 'Critical';

  const burnRateColor = burnMultiplier < 0.9 ? 'text-green-600 dark:text-green-400' :
                        burnMultiplier < 1.1 ? 'text-primary' :
                        burnMultiplier < 2 ? 'text-yellow-600 dark:text-yellow-400' :
                        'text-destructive';
      ...
      <Card>
        <CardContent className="pt-6">
          ...
          <div className="mt-4 flex items-center text-sm text-muted-foreground">
            <span>Day {daysElapsed} of {calculation.periodDays}</span>
          </div>
        </CardContent>
      </Card>
        ...
        <CardContent className="pt-6">
          <div className="flex items-start justify-between">
            <div>
              ...
              <p className={cn("text-2xl font-bold", burnRateColor)}>
                {burnMultiplier.toFixed(1)}x
              </p>
              <p className="text-sm text-muted-foreground">
                {burnRateText}
          ...
          <p className="mt-4 text-xs text-muted-foreground">
            Consuming {formatDuration(calculation.burnRate)}/day
            (sustainable: {formatDuration(calculation.sustainableRate)}/day)
          </p>
        </CardContent>
      </Card>
